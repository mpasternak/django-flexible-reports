...
A :ref:`Datasource <concept-datasource>` says *which rows* end up in a table.
...
save time instead of producing a silently empty report. See :doc:`queries`.

Step 2 -- a table
-----------------

A :ref:`Table <concept-table>` says *how* rows are rendered. It is a bag of
:ref:`Columns <concept-column>`:
...
A :ref:`Report <concept-report>` glues datasources and tables together through
:ref:`ReportElements <concept-reportelement>`:
...
* :doc:`concepts` -- every field of every model.
* :doc:`queries` -- django-dsl vs DjangoQL, parameters, shortcuts.
* :doc:`rendering` -- what the template tag does and what a column template gets.
* :doc:`exporting` -- ``.docx``, XLSX, CSV.
* :doc:`cloning` -- copy a table or a report.
