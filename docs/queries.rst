...
django-dsl_ compiles a compact boolean expression into a Django ``Q`` object:
...
DjangoQL_ is a richer, more forgiving language with an autocompleting editor
and introspection of related models:

.. code-block:: text

    pages > 300 and author.country = "Poland"

The dependency is the djangoql-iplweb_ fork, which keeps the ``djangoql``
import name and adds i18n for parser and schema error messages.
...
The context reaches the datasource through the report; see :doc:`rendering`.
