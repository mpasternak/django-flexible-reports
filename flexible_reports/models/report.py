...
class ReportElement(Titled, Orderable):
    parent = models.ForeignKey(
        "flexible_reports.Report", verbose_name=_("Parent"), on_delete=models.CASCADE
    )
    ...
    datasource = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Fill only if 'datasource' has been chosen in 'Data from' field"),
    ...
    base_model = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Fill only if 'except catchall' is selected in 'Data from' field"),
        on_delete=models.CASCADE,
    )
    ...
    def clean(self):
        ...
        if self.data_from == DATA_FROM_DATASOURCE:
            if self.datasource is None:
                raise ValidationError(
                    {"datasource": [ValidationError(_("Please specify a datasource."))]}
                )
            ...
            if self.base_model is not None:
                raise ValidationError(
                    {
                        "base_model": [
                            ValidationError(_("Please specify an empty base model."))
                        ]
                    }
                )
    ...
    def base_queryset(self):
        if self._base_queryset is None:
            raise ImproperlyConfigured(
                "Please set base queryset for this "
